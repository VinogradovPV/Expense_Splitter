...
Все YAML-файлы сохраняются в UTF-8. Текущая схема покупок — v2. YAML хранит технические значения
статусов и scope (`open`, `closed`, `reopened`, `all`) для совместимости и автоматизации. Русские
подписи применяются только в GUI и визуальных отчетах; schema ради локализации не меняется.
...
## Settlement period UX invariants

`settlement_periods.yaml` является источником истории закрытых периодов. Содержательная запись
периода имеет непустой `purchase_ids`, положительный `total_amount` и сохраненный snapshot
`settlements`. Такие записи нельзя удалять и нельзя напрямую менять их даты, покупки, сумму,
переводы или статус через GUI/CLI metadata-команды.

Пустой технический период определяется строго: `purchase_ids: []`, `total_amount: '0.00'` и
`settlements: []`. Только такие записи разрешено удалять командами `delete-empty` и
`delete-empty-all` после typed confirm. Перед записью файла storage создает timestamped backup.

Автоподстановка нового закрытия игнорирует пустые и `reopened` periods как расчетную границу.
Последний meaningful `closed` period используется только для выбора следующего `date_from`;
snapshot уже закрытых периодов при этом не пересчитывается.
