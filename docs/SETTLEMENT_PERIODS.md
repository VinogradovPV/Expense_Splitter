...
В GUI выбранный период можно открыть кнопкой `Детали периода` или переоткрыть. Reopen возвращает
его покупки в scope `open`; GUI требует отдельное подтверждение и после операции обновляет покупки,
расчёты и список периодов.
