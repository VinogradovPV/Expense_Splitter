...
Для обычного запуска на Windows используйте GUI launcher:

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\run-gui.ps1
```

GUI открывает отдельное окно без консольного меню. В нем доступны разделы:

- `Покупки` — просмотр и добавление покупок;
- `Текущие расчеты` — балансы и итоговые переводы по open scope;
- `Периоды взаиморасчетов` — просмотр периодов, preview и закрытие периода;
- `Аналитика и отчеты` — запуск аналитики за месяц, квартал или год;
- `Данные и обслуживание` — открытие папок `data` и `reports`, проверка чтения данных;
- `Справка`.

Кнопка `Закрыть период и обнулить текущие взаиморасчеты` не удаляет покупки. GUI сначала
показывает preview, затем предупреждение и диалог подтверждения. После закрытия текущие
расчеты используют только незакрытые покупки.
