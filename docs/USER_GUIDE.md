...
- `Покупки` — просмотр, добавление и редактирование покупок;
...
- `Данные и обслуживание` — открытие папок, проверка данных и безопасный reset-test;
- `Справка`.
...
### Добавление и редактирование покупок

В форме покупки участники выбираются кнопкой `Выбрать…` с действиями `Выбрать все`, `Снять все`
и `Применить`. Ручное поле со списком через запятую остается доступным. Неизвестные участники не
добавляются в справочник молча.

Категорию можно выбрать в editable ComboBox, ввести вручную или выбрать несколько категорий
галочками. Несколько значений сохраняются в строке через запятую. При вводе новой категории GUI
просит подтверждение перед добавлением в `data/categories.yaml`.

Предустановленный справочник содержит:
...
Для изменения открытой покупки выберите строку и нажмите `Редактировать покупку`. Покупки из
закрытых периодов не редактируются; сначала переоткройте период либо создайте корректирующую
покупку.

### Сброс тестовых данных

На вкладке `Данные и обслуживание` нажмите `Сбросить тестовые данные`, подтвердите предупреждение
и введите `RESET_TEST_DATA`. Приложение создаст backup-файлы, очистит покупки и периоды, обновит
GUI и сохранит участников, группы и категории.

Пути созданных timestamped `.bak` показываются после операции. Эти backups являются локальными
generated artifacts и не должны попадать в commit.

Сброс не равен закрытию периода: закрытие сохраняет историю покупок и только исключает закрытые
покупки из open scope; reset-test физически очищает тестовые покупки и историю периодов.

### Фильтры, история и отчёты в GUI

На вкладке `Покупки` можно фильтровать по `open/settled/all`, категории, диапазону дат и части
наименования. Нажатие на заголовки `Дата`, `Сумма`, `Категория`, `Плательщик` меняет сортировку.
Кнопки `Детали` и `Период покупки` показывают контекст выбранной записи.

Кнопка `Удалить покупку` предназначена для ошибочного ввода. Она работает только для open-покупок,
показывает предупреждение и требует точную строку `DELETE_PURCHASE`. Закрытые покупки не удаляются;
сначала переоткройте период либо используйте корректирующую покупку.

На вкладке `Текущие расчёты` режим `open` исключает закрытые покупки. Кнопка `Показать все с
историей` временно показывает scope `all`. На вкладке периодов доступны details и reopen.

После создания аналитики используйте `Открыть папку отчета`, `Открыть HTML`, `Открыть XLSX` или
`Открыть Markdown`. Статус последнего отчёта отображается под кнопками. Если файла нет, GUI
предлагает сначала создать его.
